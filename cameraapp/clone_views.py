...
class CameraViewSet(viewsets.ViewSet):
    def list(self, request):
        ...
        # หากไม่มีการกรอง ให้เลือกข้อมูลล่าสุดจากแต่ละ checkpoint_id
        if not any([checkpoint, start_datetime, end_datetime, plate_part1, plate_part2, plate_part3]):
            print("No filters applied, fetching latest data per checkpoint...")
            
            # ดึงข้อมูลล่าสุดจากแต่ละ checkpoint_id
            latest_per_checkpoint = (
                Camera.objects.values('checkpoint_id')
                .annotate(latest_timestamp=Max('timestamp'))
            )

            print(f"Latest per checkpoint data: {list(latest_per_checkpoint)}")

            # สร้าง queryset สำหรับข้อมูลล่าสุด
